package com.codestates.seb.V2; // 패키지 선언: 코드의 소속을 나타내며, 관리를 용이하게 합니다.
...
public class Shop {
  ...
  // 생성자: 매장 이름을 초기화하고 나머지 변수들은 initValues() 메서드를 통해 초기화합니다.
  ...
  // 이 메서드는 중복된 음식 이름이 없을 때만 새 음식을 배열에 추가하고, 성공 또는 실패를 boolean 값으로 반환합니다.
  public boolean addFood(String foodName, int price) {
    int currentIdx = -1;  // 추가할 위치를 찾기 위한 인덱스 변수
    for(int i = 0; i < foodNames.length; i++) {
      if(foodNames[i].equals(foodName)) {  // 같은 이름의 음식이 이미 등록되어 있을 경우
        System.out.println("[시스템] 같은 이름의 상품은 등록할 수 없습니다.");
        return false;  // 실패 반환
      } else if(foodNames[i].equals("")) {  // 빈 위치 찾기
        currentIdx = i;
        break;
      }
    }
  ...
  public String getShopName() {
    return shopName;  // 매장 이름 반환
  }
  public String[] getFoodNames() {
    return foodNames;  // 음식 이름 배열 반환
  }
  public int[] getPrices() {
    return prices;  // 음식 가격 배열 반환
  }
}
